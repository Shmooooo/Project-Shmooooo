...
limitRounds = int(raw_input("Stop at given number of rounds?\
 					0 for no, 1 for yes:"))
if limitRounds == True:
    roundLimit = int(raw_input("Maximum number of rounds to fight for:"))
else:
    roundLimit = 0
playerCount = int(raw_input("Number of players participating\
(anything over 300 will be slow, calculation time is probably O(2^n)):"))

swapThreshold = int(raw_input("How many more defeats than wins for\
a player to swap out gear:"))

memory = int(raw_input("Do players consider their entire win/loss\
record before swapping out (0 for no, 1 for yes):"))
...
totalWin = all([bool(players[0].wa == p.wa) for p in players])
while (totalWin == False) and ((limitRounds == False) or (j <= roundLimit)):
    j += 1
    if limitRounds == True:
...
plt.ylabel("Number of players with given combination.\n Diamond = armour, \
							triangle = weapon")

titleString = str(playerCount) + " player battle. "
titleString += " " + str(swapThreshold) + "more losses than wins to swap.\n"
if memory == True:
    titleString += "Players remember entire win/loss record.\n"
else: titleString += "Player records reset every round.\n"
if limitRounds == True:
    titleString += "Round number capped at" + str(roundLimit) + "\n"
plt.title("Weapon/armour combinations vs round of combat.\n" + titleString)
